...
打开 [EvolutionSandbox 原点演化实验](https://git-hpw.github.io/EvolutionSandbox/)。网页不连接服务器、不需要账号，也不会读取 Luanti 存档或 API 密钥。行为会经过浏览器 ESIP 控制适配器和权威适配器，状态、revision、序列与幂等记录保存在本站点 localStorage，刷新后可以继续。实现与安全边界见 [浏览器 ESIP 平台](docs/browser-platform.md)。
