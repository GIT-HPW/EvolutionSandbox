...
浏览器发布包含两个表现层：站点根路径的轻量版适合快速加载和低配置设备，`/babylon/` 是 Babylon.js 高精度版的演化纪元大厅。用户从大厅进入独立场景页面，再通过场景内“选项”暂停、保存检查点并返回大厅；页面不提供任意跨场景链接。轻量版和公开原点场景复用同一个浏览器权威适配器、ESIP 消息类型、内容包、存储键和 actor；在同一浏览器中切换客户端会继续当前进度，不会产生第二套规则状态。
...
Babylon 导航层使用独立的 `evolution-sandbox.scene-flow.v1` 本地记录，只保存当前导航状态、最近场景和场景权威端返回的轻量检查点元数据。它不能修改领域状态，也不能替代 ESIP 或确定性控制器存档。正常路径固定为阶段选择、加载、游玩、暂停、检查点和转场；直接打开场景地址、未列入当前构建白名单的场景和顺序不合法的状态切换都不会获得运行授权。导航记录损坏时不会静默覆盖，用户必须在大厅显式重建。
