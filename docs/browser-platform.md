...
首屏的动漫风宇宙由本站点内置的原生 WebGL shader 实时渲染。它只读取适配器已经确认的公开状态，用能量核心、时间环、碎片、三维网格和冲击波呈现阶段及行为，不参与规则计算，也不写入存档。渲染不下载纹理、远程脚本或模型；WebGL 不可用时会自动显示 CSS 静态景观，系统启用“减少动态效果”时只在状态变化时绘制静态帧。
